-- In-depth (consent-gated) analytics. The public tracker now asks visitors to
-- opt in before any tracking runs; when they accept it records richer, still
-- first-party signals: device class (mobile/desktop/tablet), browser and OS
-- family. Finer attributes (screen bucket, language, timezone, connection) ride
-- along in `meta` so we don't churn the schema for every dimension.
--
-- Lawful basis: every stored row now corresponds to a visitor who pressed
-- "Accept" on the consent banner (see assets/analytics.js). Visitors who decline
-- — or who haven't chosen yet — generate NO rows at all, which is why there is
-- still no IP column here: we deliberately don't persist network identifiers
-- from the client. (Consented server-side IP capture, if ever wanted, belongs in
-- an edge function with its own disclosure, not in this anon-insert path.)

alter table public.analytics_events
  add column if not exists device  text,   -- 'mobile' | 'tablet' | 'desktop'
  add column if not exists browser text,   -- 'Safari' | 'Chrome' | 'Firefox' | ...
  add column if not exists os      text;   -- 'iOS' | 'Android' | 'Windows' | 'macOS' | ...

-- Re-create the anon INSERT policy so the new dimensions are allowed and bounded.
-- Same model as before: anon may INSERT, nobody may SELECT with the public key.
...
-- Extend the admin aggregate with device / browser / OS breakdowns. Still
-- SECURITY INVOKER and service-role-only, so raw rows stay private.
...
as $$
  ...
  select jsonb_build_object(
    ...
    -- audience breakdowns from the consented dimensions (pageviews only, so each
    -- visit counts once per hit). Useful for "who/what should I make + market to".
    'devices', coalesce((
      select jsonb_agg(d order by (d->>'count')::int desc)
      from (

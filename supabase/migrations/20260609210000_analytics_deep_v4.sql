-- Deep analytics v4. The consent-gated tracker now also records listen depth
-- (which track, how far), outbound platform clicks, first-touch utm campaigns,
-- a new-visitor flag, real-user performance (web vitals + the perf governor's
-- lite mode) and the first JS errors per page. All of it rides the existing
-- type='event' rows (target = event name, details in meta), so the only schema
-- change is a size bound on meta in the INSERT policy. The aggregate gains the
-- matching breakdowns. Still SECURITY INVOKER + service-role-only: raw rows
-- stay private, the public key can only INSERT.

-- Re-create the anon INSERT policy with a cap on the meta payload, so the open
-- insert path can't be used to stuff arbitrarily large json into the table.
...
-- the event lookups below all filter type='event' + target
create index if not exists analytics_events_target_idx
  on public.analytics_events (type, target);
...
as $$
  ...
  select jsonb_build_object(
    ...
    'perf', jsonb_build_object(
      ...
      'avg_lcp',   (select round(avg((meta->>'lcp')::numeric)) from ev
                    where type = 'event' and target = 'web_vitals' and (meta->>'lcp') ~ '^[0-9.]+$'),
      'p75_lcp',   (select round(percentile_cont(0.75) within group (order by (meta->>'lcp')::numeric))
                    from ev where type = 'event' and target = 'web_vitals' and (meta->>'lcp') ~ '^[0-9.]+$'),
      'avg_cls',   (select round(avg((meta->>'cls')::numeric), 3) from ev
    ...
    'perf_by_browser', coalesce((
      ...
      from (
        select jsonb_build_object(
          ...
          'samples', count(*),
          'avg_lcp', round(avg((meta->>'lcp')::numeric) filter (where (meta->>'lcp') ~ '^[0-9.]+$')),
    ...
    'funnel', jsonb_build_object(
      ...
      'listened', (select count(distinct session_id) from ev
                   where type = 'event' and target = 'listen'
                     and (meta->>'pct') ~ '^[0-9]+$' and (meta->>'pct')::int >= 50
